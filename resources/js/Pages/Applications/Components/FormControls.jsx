...
export const inputBaseStyles = (hasError, readOnly) => `
    w-full px-4 py-2.5 text-xs font-medium text-slate-800 transition-all duration-150 outline-none
    placeholder:text-slate-400
    ${
        readOnly
            ? "bg-slate-100/70 text-slate-600 border border-slate-200"
            : hasError
              ? "border border-rose-300 bg-rose-50/30 focus:border-rose-500 focus:ring-2 focus:ring-rose-500/10"
              : "border border-slate-200 bg-white hover:border-slate-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10 shadow-2xs"
    }
`;

export const Input = ({ className = "", hasError = false, readOnly = false, ...props }) => (
    <input className={`${inputBaseStyles(hasError, readOnly)} rounded-full ${className}`} readOnly={readOnly} {...props} />
);
