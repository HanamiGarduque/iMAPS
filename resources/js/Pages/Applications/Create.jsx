...
export default function Create({ auth, errors: serverErrors = {} }) {
    ...
    const lookupPin = async (pin) => {
        ...
        if (!res.ok) throw new Error(payload.message || "PIN not found");
        return payload.data;
    };

    const handlePinLookup = async (index) => {
        const pin = form.parcels[index]?.property_index_number;
        ...
        try {
            ...
            if (!form.barangay && data.barangay) {
                setForm((prev) => ({ ...prev, barangay: data.barangay }));
            }
        ...
        if (step === 3) {
            ...
            if (!form.parcels.length) {
                ...
                form.parcels.forEach((parcel, index) => {
                    if (!parcel.property_index_number?.trim()) {
                    ...
                    if (!parcel.lot_number?.trim() && !parcel.tct_number?.trim() && !parcel.tax_dec_number?.trim()) {
                        newErrors[`parcels.${index}.lot_number`] = "Please lookup PIN to fetch identifier";
                    }

                    if (!parcel.lot_area_sqm || Number(parcel.lot_area_sqm) <= 0) {
                        newErrors[`parcels.${index}.lot_area_sqm`] = "Lookup PIN to fetch declared area";
                    }
                });
            }
        }
    ...
    const handleNext = () => {
        if (validateStep(currentStep)) {
            ...
            setFlash({
                type: "error",
                msg: "Please complete highlighted fields.",
            });
        ...
        router.post("/applications/encode", form, {
            ...
            onError: (errs) => {
                ...
                if (errs.application_type || errs.land_use_class || errs.purpose) {
                    ...
                    errs.lot_area_sqm ||
                    errs.coordinates ||
                    Object.keys(errs).some((k) => k.startsWith("parcels"))
                ) {
                    setCurrentStep(3);
                } else {
                    setCurrentStep(5);
                    ...
                    <div className="hidden lg:flex flex-1 max-w-md mx-6">
                        <div className="relative w-full">
                            ...
                            <input
                                ...
                                className="block w-full pl-8 pr-10 py-1.5 border border-slate-200 rounded-lg text-xs bg-slate-50 focus:bg-white focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10 transition-all outline-none"
                            />
                        </div>
                    </div>
                    ...
                    <main className="flex-1 w-full h-full flex flex-col transition-all duration-500 ease-in-out bg-[#f8fafc]" style={{ paddingLeft: sidebarOpen ? "200px" : "0px" }}>
                        <div className="p-3 md:p-5 flex-1 flex flex-col h-full overflow-hidden">
                            ...
                            <div className="max-w-[1000px] mx-auto w-full mb-3 flex-shrink-0 flex items-center justify-between gap-4">
                                ...
                                {flash && (
                                    <div
                                        className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border shadow-sm ${flash.type === "success" ? "bg-emerald-50 border-emerald-200 text-emerald-800" : "bg-red-50 border-red-200 text-red-800"}`}
                                    >
                                        {flash.type === "success" ? (
                                        ...
                                        <button onClick={() => setFlash(null)} className="text-slate-400 hover:text-slate-800">
                                        ...
                                        {STEPS.map((step) => {
                                            ...
                                            return (
                                                <div key={step.id} className="relative pl-5 cursor-pointer group" onClick={() => isCompleted && setCurrentStep(step.id)}>
                                                    ...
                                                    <div
                                                        className={`absolute left-[-13px] top-0 w-6 h-6 rounded-full flex items-center justify-center transition-all duration-300 z-10 border-[1.5px]
                                                        ${
                                                            isCompleted
                                                                ? "bg-blue-500 border-blue-500 text-white"
                                                                : isCurrent
                                                                  ? "bg-white border-blue-600 text-blue-600 shadow-[0_0_0_3px_rgba(37,99,235,0.15)] scale-110"
                                                    ...
                                                    <div>
                                                        <p
                                                            className={`text-[12px] font-bold tracking-tight transition-colors leading-tight ${isCurrent ? "text-blue-700" : isCompleted ? "text-slate-800" : "text-slate-400 group-hover:text-slate-600"}`}
                                                        >
                                                            {step.title}
                                                        </p>
                                        ...
                                        {currentStep === 1 && (
                                            <div className="form-enter flex-1 flex flex-col">
                                                ...
                                                <div className="flex-1 flex flex-col gap-y-4">
                                                    <div>
                                                        <Label required hasError={!!errors.application_type}>
                                                            ...
                                                            {APPLICATION_TYPES.map((type) => (
                                                                <label key={type.id} className="radio-card relative cursor-pointer group">
                                                                    <input
                                                                        ...
                                                                        name="app_type"
                                                                        value={type.id}
                                                                        checked={form.application_type === type.id}
                                                    ...
                                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                                                        <div className="relative">
                                                            <Label required hasError={!!errors.form_number}>
                                                        ...
                                                        <div className="relative">
                                                            <Label required hasError={!!errors.land_use_class}>
                                                                Target Zoning Class
                                                            </Label>
                                                    ...
                                                    <div className="relative flex-1 flex flex-col">
                                                        <Label required hasError={!!errors.purpose}>
                                                            Operational Purpose
                                                        </Label>
                                                        <Textarea
                                                            rows={2}
                                                            ...
                                                            placeholder="Explicitly detail the intended use of the land or structure..."
                                        ...
                                        {currentStep === 2 && (
                                            <div className="form-enter flex-1 flex flex-col">
                                                ...
                                                <div className="space-y-4 flex-1">
                                                    <div className="relative">
                                                        <Label required hasError={!!errors.applicant_name}>
                                                            Registered Applicant / Corp
                                                        </Label>
                                                        <Input
                                                            type="text"
                                                    ...
                                                    <div className="grid grid-cols-2 gap-4">
                                                        <div className="relative">
                                                            <Label required hasError={!!errors.contact_number}>
                                                                Primary Phone
                                                            </Label>
                                                            <div className="relative">
                                                                <span className="absolute inset-y-0 left-0 flex items-center pl-3 text-slate-400 font-mono text-[12px] font-bold pointer-events-none">
                                                                    +63
                                                                </span>
                                                                <Input
                                                                    type="tel"
                                            ...
                                            <div className="form-enter flex-1 flex flex-col">
                                                ...
                                                <div className="flex-1 flex flex-col gap-4 overflow-y-auto pr-1">
                                                    <div className="rounded-[12px] border border-slate-200 bg-slate-50/80 p-4 shadow-sm">
                                                        <div className="mb-4">
                                                            <Label required hasError={!!errors.barangay}>
                                                                ...
                                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-3 gap-y-4">
                                                                    <div className="relative md:col-span-2">
                                                                        ...
                                                                        <div className="flex flex-wrap items-center gap-2">
                                                                            ...
                                                                            <button
                                                                                ...
                                                                                className={`inline-flex items-center justify-center rounded-[10px] px-3 py-2 text-[11px] font-bold transition ${
                                                                            ...
                                                                            >
                                                                                {pinLoading[index] ? "Looking up..." : "Lookup"}
                                                                            </button>
                                                                        </div>
                                                                    ...
                                                                    <div className="relative">
                                                                        ...
                                                                        <Input
                                                                            ...
                                                                            placeholder="Auto-filled via lookup"
                                                                    ...
                                                                    <div className="relative md:col-span-2">
                                                                        <Label required hasError={!!errors[`parcels.${index}.lot_area_sqm`]}>
                                                                            Declared Floor Area
                                                                        </Label>
                                                                        <div className="relative">
                                                                            <Input
                                                                            ...
                                                                            />
                                                                            <span className="absolute inset-y-0 right-0 flex items-center pr-3 text-slate-400 font-bold text-[9px] pointer-events-none">
                                                                                SQM
                                                                            </span>
                                                                        </div>
                                                ...
                                                <div className="flex-1 overflow-y-auto space-y-3 pr-2">
                                                    ...
                                                    <div className="bg-slate-50 border border-slate-200 rounded-[10px] p-3.5 relative group">
                                                        <button
                                                            ...
                                                            className="absolute top-3 right-3 text-[10px] font-bold text-blue-600 hover:text-blue-800 transition-colors uppercase tracking-wider flex items-center gap-1"
                                                        >
                                                            <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
                                                                <path
                                                                    strokeLinecap="round"
                                                        ...
                                                        <div className="grid grid-cols-2 gap-2.5">
                                                            <div className="col-span-2">
                                                                ...
                                                                <p className="text-[12px] font-bold text-slate-800">
                                                                    {form.barangay ? `Brgy. ${form.barangay}` : "—"} {form.street_address ? `(${form.street_address})` : ""}
                                                                </p>
                                                            </div>
                                                            <div>
                                                                <p className="text-[9px] font-semibold text-slate-500 uppercase tracking-widest mb-0.5">Property Index</p>
                                                                <p className="text-[11px] font-mono text-slate-800 uppercase">{form.parcels[0]?.property_index_number || "—"}</p>
                                                            </div>
                                                            <div>
                                            ...
                                            <div className="form-enter flex-1 flex flex-col">
                                                ...
                                                <div className="flex-1 flex flex-col justify-center max-w-md mx-auto w-full">
                                                    <div className="bg-blue-50 border-2 border-blue-100 rounded-[14px] p-5 shadow-sm">
                                                        ...
                                                        <div className="space-y-3">
                                                            <div className="relative">
                                                                <Label required hasError={!!errors.assessment_fee}>
                                                                    Calculated Fee
                                                                </Label>
                                                                <div className="relative">
                                                                    <span className="absolute inset-y-0 left-0 flex items-center pl-3 text-slate-400 font-mono text-[14px] font-black pointer-events-none">
                                                                        ₱
                                                                    </span>
                                                                    <Input
                                                                        type="number"
                                                                        ...
                                                                        step="0.01"
                                                                        placeholder="0.00"
                                                                        className="pl-8 font-mono font-black text-[15px] text-blue-900 bg-white"
